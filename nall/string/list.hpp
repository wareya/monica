...
auto lstring::find(const string& source) const -> maybe<unsigned> {
  for(unsigned n = 0; n < size(); n++) {
...
auto lstring::ifind(const string& source) const -> maybe<unsigned> {
  for(unsigned n = 0; n < size(); n++) {
...
auto lstring::match(const string& pattern) const -> lstring {
  lstring result;
  for(unsigned n = 0; n < size(); n++) {
...
auto lstring::merge(const string& separator) const -> string {
  string output;
  for(unsigned n = 0; n < size(); n++) {
    output.append(operator[](n));
    if(n < size() - 1) output.append(separator);
  }
  return output;
}
