...
struct lstring : vector<string> {
  ...
  inline auto find(const string& source) const -> maybe<unsigned>;
  inline auto ifind(const string& source) const -> maybe<unsigned>;
  inline auto match(const string& pattern) const -> lstring;
  inline auto merge(const string& separator) const -> string;
  inline auto strip() -> type&;
